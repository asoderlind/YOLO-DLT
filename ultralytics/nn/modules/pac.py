...
class PacConv2dFn(Function):
    ...
    def forward(ctx, input, kernel, weight, bias=None, stride=1, padding=0, dilation=1, shared_filters=False):
        ...
        # Reshape for multiplication with kernel
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0] and input is not None:
            if ctx.shared_filters:
                grad_in_mul_k = grad_output.view(bs, out_ch, 1, 1, out_sz[0], out_sz[1]) * weight.view(
            ...
            grad_in_mul_k = grad_in_mul_k.view(bs, -1, out_sz[0] * out_sz[1])

            # Use fold operation to reconstruct the input gradient
            grad_input = F.fold(
                grad_in_mul_k,
                ctx.input_size,
                ctx.kernel_size,
        ...
        # Calculate gradients for kernel and weight
        if (ctx.needs_input_grad[1] or ctx.needs_input_grad[2]) and input is not None:
            ...
            if ctx.needs_input_grad[1] and kernel is not None:
                if ctx.shared_filters:
                    grad_in_mul_k = grad_output.view(bs, out_ch, 1, 1, out_sz[0], out_sz[1]) * weight.view(
                        ctx.kernel_size[0], ctx.kernel_size[1], 1, 1
                    )
...
class GaussKernel2dFn(Function):
    ...
    def forward(ctx, input, kernel_size, stride, padding, dilation, channel_wise):
        ...
        # Sum across channels if not channel-wise
        if not channel_wise:
    ...
    def backward(ctx, grad_output):
        ...
        # Unfold input again for backward pass
        cols = F.unfold(input, ctx.kernel_size, ctx.dilation, ctx.padding, ctx.stride)
        cols = cols.view(bs, ch, ctx.kernel_size[0], ctx.kernel_size[1], out_h, out_w)
        ...
        # Handle center pixel gradient
        grad_diff[:, :, center_y : center_y + 1, center_x : center_x + 1, :, :] -= grad_diff.sum(
            dim=2, keepdim=True
        ).sum(dim=3, keepdim=True)

        # Reshape grad_diff for folding
        grad_diff = grad_diff.view(bs, ch * ctx.kernel_size[0] * ctx.kernel_size[1], -1)

        # Use fold operation instead of Im2Col_updateGradInput
        # For the backward pass of unfold, we need to use fold
        grad_input = F.fold(
            grad_diff, (in_h, in_w), ctx.kernel_size, dilation=ctx.dilation, padding=ctx.padding, stride=ctx.stride
        )
...
def pacconv2d(
    input, kernel, weight, bias=None, stride=1, padding=0, dilation=1, shared_filters=False, native_impl=False
):
...
def packernel2d(
    ...
    if transposed:
        in_sz = tuple(
            ...
            for (o, k, s, p, op, d) in zip(input.shape[-2:], kernel_size, stride, padding, output_padding, dilation)
        )
    else:
        in_sz = input.shape[-2:]
    ...
    if mask is not None or normalize_kernel:
        ...
        if mask is not None:
            ...
            if not normalize_kernel:
                norm = mask.sum(dim=2, keepdim=True).sum(dim=3, keepdim=True) / mask_pattern.sum(
                    dim=2, keepdim=True
                ).sum(dim=3, keepdim=True)
        else:
            mask = mask_pattern
    ...
    if native_impl:
        ...
        if smooth_kernel_type == "none":
            ...
            smoothed = F.conv2d(
                input_for_kernel_crop, smooth_kernel, stride=stride, padding=tuple(np.maximum(0, smooth_padding))
            )
