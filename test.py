...
def train_with_timing(model_path, data_path, batch_size, epochs, device, name, use_fe=False, augment=True, optimizer="AdamW", pretrained=False):
    ...
    # Train the model
    results = model.train(data=data_path, batch=batch_size, epochs=epochs, device=device, pretrained=pretrained, optimizer=optimizer, use_fe=use_fe, augment=augment, name=name) 
...
train_with_timing(
        model_path="yolo11l.yaml",
        data_path="bdd100k.yaml",
        batch_size=16,
        epochs=200,
        device="cuda",
        name="bdd100k-yolo11l",
        optimizer="auto"
        )
